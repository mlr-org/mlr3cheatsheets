	...
	\begin{columns}
		...
		\begin{column}{.245\textwidth}
            ...
            \begin{myblock}{Hyperparameters}
              ...
              \ \\
              The parameters of all \codeinline{PipeOp}s and \codeinline{Learner}s in a \codeinline{Graph} can be jointly tuned. This can be done as with any other \codeinline{Learner} (see \href{FIXME:CheatsheetLink}{mlr3tuning}). Hyperparameter names are prefixed with the respective \codeinline{PipeOp}'s \codeinline{id}, e.g., \codeinline{pca.rank.} corresponds to the \codeinline{PipeOpPCA}'s \codeinline{rank.} parameter.
            \end{myblock}
       			\vfill}
				\end{minipage}
			\end{beamercolorbox}
