				...
				\begin{minipage}{.98\textwidth}
					\parbox[t][\columnheight]{\textwidth}{
						...
						\begin{myblock}{ParamterSet}
							...
							before passing them to the \codeinline{Tuner}.
						\end{myblock}
						\vfill}
				\end{minipage}
			\end{beamercolorbox}
				...
				\begin{minipage}{.98\textwidth}
					\parbox[t][\columnheight]{\textwidth}{
						...
						\begin{myblock}{Triggering the tuning}
							...
							\\
							\\
							The \codeinline{Tuner} generates hyperparameter configurations 
							and passes them to \codeinline{TuningInstance} by internally calling \codeinline{\$eval()} 
							until the budget of \codeinline{Terminator} is exhausted (5). 
							To access the results use the following methods of \codeinline{TuningInstance}:
							\\
							\begin{codebox}
								instance\$\textbf{archive}(unnest)
							\end{codebox}
							Returns all resampling results together with the used hyperparameters. Use \codeinline{unnest} to display hyperparameter without (\codeinline{tune\_x}) or with (\codeinline{params}) trafo applied.
							\\
							\begin{codebox}
