...
@media print {
  ...
  div.sourceCode:has(> pre.nested_narrow) {
    width: 94% !important;
  }
}
</style>
...
```{r, results='hide'}
instance = ti(task, learner, resampling, measure, terminator, ss)
```
...
instance = ti(task, learner, resampling, measure, evals, ss)
...
Change log-level for mlr3tuning (uses `bbotk` logger internally).
...
instance = tune(tnr("grid_search", resolution = 5), task = tsk("iris"),
  learner = learner, resampling = rsmp("holdout"),
...
```{r, results='hide'}
at = auto_tuner(tuner = tuner, learner = learner,
...
```{r, results='hide', class.source='example nested_narrow'}
inner = rsmp("holdout")
...
```{r, results='hide', class.source='multiline nested_narrow'}
extract_inner_tuning_results(rr)
# >    iteration     cost      gamma classif.ce ...
# > 1:         1 1.222198 -0.4974749       0.08 ...
```
...
```{r, results='hide', class.source='nested_narrow'}
rr$aggregate()
# > classif.ce
# >       0.04
```
...
<!-- ht<span>tps prevents the plain-text URL from being auto-linked -->
* [mlr3book](https://mlr3book.mlr-org.com)<br>
(ht<span>tps://mlr3book.mlr-org.com)
* [mlr-org on GitHub](https://github.com/mlr-org)<br>
(ht<span>tps://github.com/mlr-org)
* [mlr3tuning](https://github.com/mlr-org/mlr3tuning)<br>
(ht<span>tps://github.com/mlr-org/mlr3tuning)
