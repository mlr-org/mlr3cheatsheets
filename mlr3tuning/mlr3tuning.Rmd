...
@media print {
  ...
  pre.sourceCode.r.nested_small > code.sourceCode.r.example_code,
  pre.sourceCode.r.nested_small > code.sourceCode.r {
...
instance = tune(tnr("grid_search", resolution = 5), task = tsk("iris"),
  learner = learner, resampling = rsmp("holdout"), measures = msr("classif.ce"))
```
Use `tune()`-shortcut.
...
```{r, results='hide', class.source='multiline nested_small'}
at$tuning_result
# >        cost     gamma learner_param_vals  x_domain classif.ce
...
```{r, results='hide', class.source='nested_small'}
...
```{r, results='hide', class.source='example nested_small nested_narrow'}
inner = rsmp("holdout")
...
```{r, results='hide', class.source='multiline nested_small nested_narrow'}
...
```{r, results='hide', class.source='nested_small nested_narrow'}
rr$aggregate()
#> classif.ce 
#>       0.04 
```
