...
```{r, results='hide'}
instance = FSelectInstanceBatchSingleCrit$new(task, 
...
instance = FSelectInstanceBatchSingleCrit$new(task, learner, resampling, measure, 
  terminator)
...
Use `FSelectInstanceBatchMultiCrit` for multi-criteria feature selection.
...
task_selected$select(instance$result_feature_set)
```

Set optimized feature subset in a cloned `Task`.

```{r, results='hide', class.source='example'}
instance = fselect(fs("random_search"), task = tsk("iris"), learner = learner, 
  resampling = rsmp("holdout"), measures = msr("classif.ce"), term_evals = 20)
```
...
```{r, results='hide'}
afs = AutoFSelector$new(fselector = fselector, learner = learner,
  resampling = resampling, measure = measure, terminator = terminator)
```
...
```{r, results='hide'}
afs = auto_fselector(fs("random_search"),
...
```{r, results='hide', class.source='example'}
inner = rsmp("holdout")
afs = auto_fselector(fs("random_search"), learner, inner, measure, term_evals = 20)
...
```{r, results='hide'}
rr = fselect_nested(fs("random_search"), task, 
